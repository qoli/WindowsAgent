...
| [Evidence recorder runtime](evidence-recorder-runtime.md) | Partially landed | Record an independent one-slot-per-second 1080p timeline, export authenticated UTC ranges, and build exact-timestamp contact sheets from committed MP4 evidence. |
...
| [Action OSD](action-osd.md) | Landed | Project the current Streaming Action or active Ephemeral Action Sequence child and its latest three explicit activity records into a non-activating Windows overlay. |
