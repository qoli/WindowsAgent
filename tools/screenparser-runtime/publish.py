#!/usr/bin/env python3
"""Publish the pinned ScreenParser DirectML runtime as a verified Windows bundle."""
...
RUNTIME_ID = "screenparser-onnx-dml-v1"
RUNTIME_FILENAME = "ScreenParser.DirectML.exe"
...
def run(argv: Sequence[str]) -> int:
    ...
    project = Path(__file__).resolve().parents[2] / "runtimes/screenparser-directml/ScreenParser.DirectML/ScreenParser.DirectML.csproj"
    if not project.is_file():
        raise PublishError(f"runtime project does not exist: {project}")

    with tempfile.TemporaryDirectory(prefix="screenparser-directml-publish-") as temporary:
        command = [
            str(dotnet),
            "publish",
            str(project),
