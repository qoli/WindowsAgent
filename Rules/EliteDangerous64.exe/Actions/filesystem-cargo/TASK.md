...
`elite-dangerous/filesystem/cargo` is a finite query Action whose sole information source is
`Cargo.json` under the current Windows user's resolved Saved Games known
folder at `Frontier Developments/Elite Dangerous`.

The caller supplies no filename or path. The package performs exactly one
bounded strict-JSON read of `Cargo.json`, requires the event discriminator to
be one of `Cargo`, requires a valid source timestamp, and returns
