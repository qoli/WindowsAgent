...
def main(ctx):
    ...
    for hop in hops:
        ...
        if hop_index < jump_count:
            emit_update("ARRIVAL_CLEARANCE", hop_index, jump_count, from_system=current_system, to_system=target_system, next_system=next_system, route_id=route_id, child_action="elite-dangerous/clear-hyperspace-occlusion", commanded_throttle=0, fuel_main=final_readiness["fuelMain"], status_timestamp=final_readiness["sourceTimestamp"], reason="TWO_FRESH_SPHERE_DIRECTIONS+6400MS_FIXED_TURN+30000MS_SEPARATION")
            clearance = action.call(id="elite-dangerous/clear-hyperspace-occlusion", inputs={"targetName": next_system, "startMode": "SUPERCRUISE"})
            if not clearance["completed"] or clearance["entryAlignmentEvidence"] != "EXISTING_SUPERCRUISE_FIXED_SPHERE_SEPARATION" or not clearance["fixedOutwardTurnCompleted"] or clearance["fixedTurnDurationMs"] != 6400 or clearance["supercruiseEscapeDurationMs"] != 30000 or not clearance["finalSupercruiseConfirmed"] or clearance["finalCommandedThrottle"] != 0:
