...
arrival-star clearance before it begins the next hop. The child uses the same
mechanical sphere-separation core as LOS recovery: require two fresh compatible
robust-circle directions at 0%, execute a fixed 6400 ms outward turn, fly that
heading at 100% for 30 seconds, then return to 0% and confirm Supercruise. This lowers
the next alignment and charge failure probability by moving the ship away from
the arrival star before the next hyperspace target is pursued. Detector absence
is never `CLEAR` and cannot authorize thrust; fixed mechanical completion,
Supercruise, and 0%-throttle postconditions are all required.
