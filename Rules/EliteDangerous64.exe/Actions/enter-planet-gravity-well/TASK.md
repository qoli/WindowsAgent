...
`Status.json` evidence for normal space and an exact matching Destination,
visually confirms Mass Lock, Landing Gear, and Cargo Scoop OFF, and starts from
0% throttle. It first performs a bounded zero-throttle FSD probe. If the Escape
...
live WGC-pressure control for the probe.
...
evidence completes the Action. Any failure or cancellation leaves a registered
