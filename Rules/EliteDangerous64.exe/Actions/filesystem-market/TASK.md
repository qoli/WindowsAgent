...
`elite-dangerous/filesystem/market` is a finite query Action whose sole information source is
`Market.json` under the current Windows user's resolved Saved Games known
folder at `Frontier Developments/Elite Dangerous`.

The caller supplies no filename or path. The package performs exactly one
bounded strict-JSON read of `Market.json`, requires the event discriminator to
be one of `Market`, requires a valid source timestamp, and returns
